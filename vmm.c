...
void *mm_malloc(size_t size) {
   ...
   struct meta_data* split_block;
   struct meta_data* cur=head->next;
    do {
        if (head->next==head) {
            void* valid = sbrk(size+meta_data_size);
            block = (struct meta_data*)start_heap;
            block->size = size;
            block->free = 'o';
            list_add_tail(block,head);
            return ( void*)(block + 1);
        }

       // printf("%d\n", cur->next->next->next->size);
        if(cur->free=='f' && cur->size>=size) {
           
            if ((cur->size - size) >= meta_data_size) {

                split_block = (void*)cur + size + meta_data_size;
                split_block->size = cur->size - size - meta_data_size;
                split_block->free = 'f';
                cur->size = size;
                list_add(split_block,cur,cur->next);
                
            }
            //printf("%d", cur->size);
            cur->free = 'o';
            void* p = (void*)cur + meta_data_size;
        // memset(p, 0, size);
            return p;
        ...
        if (cur->next == head) {
            if ((cur->size < size && cur->free=='f')|| cur->free=='o') {
                block = (struct meta_data*)sbrk(meta_data_size + size);
                block->size = size;
                block->free = 'o';
                list_add_tail(block, head);
                return (block + 1);
...
void mm_free(void *p) {
    ...
    struct meta_data* cur = head->next;
    while (cur != head) {
        if ((struct meta_data*)(p - meta_data_size) == (struct meta_data*)cur) {
            cur->free = 'f';
            return;
        }
        cur = cur->next;
