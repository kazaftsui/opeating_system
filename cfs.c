...
void init_cfs_process() {
    ...
    for (int j = 0; j < num_process; ++j)
        sumweight += nice_to_weight(nice_value[j]);
    for (int i = 0; i < num_process; ++i) {
        process[i].weight = nice_to_weight(nice_value[i]);
        process[i].remain_time = burst_time[i];
        process[i].vruntime = 0.00;
        if (calculate_per_process_time_slice(nice_to_weight(nice_value[i]), sched_latency, sumweight) < min_granularity)
            process[i].time_slice = min_granularity;
        else
            process[i].time_slice = calculate_per_process_time_slice(nice_to_weight(nice_value[i]), sched_latency, sumweight);
    }
...
void run_cfs_scheduling() {
    ...
    while (finish_process_count!=num_process)
    {
        int x = 0;
        int choice = 0;
        while (process[x].remain_time == 0)
            x++;
        double min = process[x].vruntime;
        choice = x;
        for (int i = x; i < num_process; ++i) {
            if (process[i].vruntime < min && process[i].remain_time != 0) {
                min = process[i].vruntime;
                choice = i;
            }
        }
        if (process[choice].remain_time >= process[choice].time_slice) {
            gantt_chart_append_item(choice, process[choice].time_slice);
            process[choice].vruntime=calculate_new_vruntime(process[choice].vruntime, process[choice].time_slice, process[choice].weight);
            process[choice].remain_time -= process[choice].time_slice;
        }
        else {
            gantt_chart_append_item(choice, process[choice].remain_time);
            process[choice].vruntime = calculate_new_vruntime(process[choice].vruntime, process[choice].remain_time, process[choice].weight);
            process[choice].remain_time -= process[choice].remain_time;
        }
        if (process[choice].remain_time == 0)
